...
(Chapters~\ref{ch:gaussian} and~\ref{ch:em-results}).
...
($12.3\%$ against $21.0\%$ at $t = 0.05$)
(Chapters~\ref{ch:gaussian-bp} and~\ref{ch:em-results}).
...
baseline given locality and weight sharing,
$\structratiolo$--$\structratiohi\times$ over the \structsizes{} sizes of the
all-site protocol.
...
long-range one, against the convolutional baseline at a coupling strength of
roughly one tenth and against both at one fifth. The convergence
...
\begin{enumerate}
  ...
  from $C = \capcappedfrom$ upward stops at
