...
% striking the top edge of every [t] float.
...
% reference and citation is clickable, and the reader can now see that it is.
...
\hypersetup{
  ...
  pdftitle={Belief Propagation for Generative Diffusion on Dynamic Objects},
...
% Grid resolution. Chapters 6 and 8 wrote this as a bare M, which collided
% twice: with the Markov transition kernel M of Chapter 2 (used again in
% Chapter 6's one-step proposition) and with the shared figures, whose axes
% are labelled N_g because the paper writes M for the training-set size.
% shared/notation.tex already renders it this way, so this alias makes the
% figures agree with the prose in both documents.
\providecommand{\ngrid}{N_{\mathrm{g}}}
% Conditional covariance. shared/notation.tex defines this; the thesis carries
% its own notation block, and building without it silently typeset
% \Cov(A\mid h) as "(A | h)" wherever Overleaf's nonstop mode dropped the
% undefined macro (Chapter 4's Hessian derivation).
...
% This volume IS the companion compendium now. The two macros below used to
% point at the earlier technical compendium (background derivations,
% experiment ledger, claim audit), which is superseded and kept in
% archive/; the phrase is retargeted so those references stay truthful.
\newcommand{\compendium}{the archived technical compendium}
\newcommand{\Compendium}{The archived technical compendium}
